...
export default function ComparisonForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (urlsToAnalyze.length < 2) {
      setError("Veuillez entrer au moins deux URLs valides pour la comparaison")
      return
    }
    ...
    try {
      ...
      for (const url of validatedUrls) {
        try {
          ...
          throw new Error(
            `Erreur lors de l'analyse de ${url}: ${error instanceof Error ? error.message : "Erreur inconnue"}`,
          )
        }
      }
      ...
      console.error(err)
      setError(
        err instanceof Error ? err.message : "Erreur lors de l'analyse. Veuillez vérifier les URLs et réessayer.",
      )
    } finally {
      ...
      <Card className="p-6 shadow-lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-3">
            {urls.map((url, index) => (
              <div key={index} className="flex flex-col sm:flex-row gap-3">
                ...
                {index === urls.length - 1 && (
                  <Button type="button" variant="outline" size="icon" onClick={handleAddUrl} disabled={isAnalyzing}>
          ...
          <div className="flex justify-between items-center">
            <Link href="/" className="text-emerald-600 hover:text-emerald-700 flex items-center text-sm">
