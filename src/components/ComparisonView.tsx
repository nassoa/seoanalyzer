    ...
    if (metric === "lighthouse") {
      return results.findIndex(
        (result) =>
          ...
          Math.max(...results.map((r) => r.lighthouseScore))
      );
    } else if (metric === "wordCount") {
      return results.findIndex(
        (result) =>
          result.wordCount === Math.max(...results.map((r) => r.wordCount))
      );
    } else if (metric === "readability") {
      return results.findIndex(
        (result) =>
          result.readabilityScore ===
          Math.max(...results.map((r) => r.readabilityScore))
      );
      ...
      return results.findIndex(
        (result) =>
          ...
          Math.max(
            ...results.map((r) => r.images.withAlt / (r.images.total || 1))
          )
      );
    } else if (metric === "links") {
      return results.findIndex(
        (result) =>
          result.links.total === Math.max(...results.map((r) => r.links.total))
      );
      ...
      return results.findIndex(
        (result) =>
          ...
          Math.max(
            ...results.map((r) => r.headings.h1.length + r.headings.h2.length)
          )
      );
    }
    return -1;
        ...
        <TabsContent value="overview" className="mt-4">
          <div className="space-y-6">
                    ...
                    {results.map((result, index) => (
                      <div
                        ...
                        className={`text-center ${
                          getBestSite("lighthouse") === index ? "font-bold" : ""
                        }`}
                      >
                        <span
                  ...
                  <div className="flex justify-between">
                    {results.map((result, index) => (
                      <div
                        key={index}
                        className={`text-center ${
                          getBestSite("wordCount") === index ? "font-bold" : ""
                        }`}
                      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Card
                className={`cursor-pointer ${
                  selectedMetric === "images" ? "ring-2 ring-emerald-500" : ""
                }`}
                    ...
                    {results.map((result, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className={`text-center ${
                            getBestSite("images") === index ? "font-bold" : ""
                          }`}
                        >
                          <span
                            className={`text-xl ${getScoreColor(
              ...
              <Card
                className={`cursor-pointer ${
                  selectedMetric === "links" ? "ring-2 ring-emerald-500" : ""
                }`}
                  ...
                  <div className="flex justify-between">
                    {results.map((result, index) => (
                      <div
                        key={index}
                        className={`text-center ${
                          getBestSite("links") === index ? "font-bold" : ""
                        }`}
                      >
              ...
              <Card
                className={`cursor-pointer ${
                  selectedMetric === "headings" ? "ring-2 ring-emerald-500" : ""
                }`}
                  ...
                  <div className="flex justify-between">
                    {results.map((result, index) => (
                      <div
                        key={index}
                        className={`text-center ${
                          getBestSite("headings") === index ? "font-bold" : ""
                        }`}
                      >
                    ...
                    {results.map((result, index) => (
                      <div key={index} className="space-y-1">
                        <div className="flex justify-between text-sm">
                          ...
                          <span
                            className={getScoreColor(result.lighthouseScore)}
                          >
                            {result.lighthouseScore}
                          </span>
                        </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {results.map((result, index) => (
                      <div key={index} className="space-y-1">
                        ...
                        <Progress
                          value={Math.min((result.wordCount / 1000) * 100, 100)}
                    ...
                    {results.map((result, index) => {
                      ...
                      return (
                        <div key={index} className="space-y-1">
                          ...
                          <Progress
                            value={altPercentage}
                            className={getProgressColor(altPercentage)}
                          />
                        </div>
                      );
                    })}
                      ...
                      <div key={index} className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          <Badge className="bg-blue-500">
                            ...
                            {Math.round(
                              (result.links.internal / result.links.total) * 100
                            )}
                            %)
                          </Badge>
                          <Badge className="bg-purple-500">
                            ...
                            {Math.round(
                              (result.links.external / result.links.total) * 100
                            )}
                            %)
                          </Badge>
                          {result.links.broken > 0 && (
                            <Badge className="bg-red-500">
                              ...
                              {Math.round(
                                (result.links.broken / result.links.total) * 100
                              )}
                              %)
                            </Badge>
                          )}
                    ...
                    {results.map((result, index) => (
                      <div key={index} className="space-y-2">
                        <div className="flex justify-between text-sm">
                          ...
                          <span>
                            H1: {result.headings.h1.length}, H2:{" "}
                            {result.headings.h2.length}, H3:{" "}
                ...
                {results.length > 1 ? (
                  <div>
                    <p className="text-sm mb-4">
                      Mots-clés qui apparaissent dans plusieurs sites analysés :
                    </p>
                  ...
                  <p className="text-sm text-gray-500">
                    Analysez au moins deux sites pour voir les mots-clés communs
                  </p>
                )}
              </CardContent>
            </Card>
                    ...
                    <tbody>
                      {results.map((result, index) => (
                        <tr key={index} className="border-b">
                          ...
                          <td className="py-2">
                            {result.metaTags.ogTitle &&
                            result.metaTags.ogDescription ? (
                              <Check className="h-4 w-4 text-green-500" />
                            ) : result.metaTags.ogTitle ||
                              result.metaTags.ogDescription ? (
                      ...
                      <div className="space-y-2">
                        ...
                        {result.recommendations.filter(
                          ...
                          <p className="text-sm text-green-500">
                            Aucun problème critique détecté pour ce site
                          </p>
                        )}
                      </div>
                    </div>
            ...
            <Card>
              ...
              <CardContent>
                {results.length > 1 ? (
                  <div className="space-y-4">
                    <p className="text-sm">
                      Problèmes qui apparaissent dans plusieurs sites :
                    </p>
                    <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-500">
                    Analysez au moins deux sites pour voir les problèmes communs
                  </p>
                )}
              </CardContent>
            </Card>
