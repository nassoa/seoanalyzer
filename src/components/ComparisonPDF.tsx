...
const PDFDocument = ({ results }: { results: SEOAnalysisResult[] }) => {
  ...
  const getBestScore = (metric: keyof SEOAnalysisResult | "altPercentage" | "h1Count") => {
    if (metric === "lighthouseScore") {
      ...
      return Math.max(
        ...results.map((r) => (r.images.total > 0 ? Math.round((r.images.withAlt / r.images.total) * 100) : 0)),
      )
    } else if (metric === "h1Count") {
      return results.findIndex((r) => r.h1Count === 1) !== -1 ? 1 : Math.min(...results.map((r) => r.h1Count))
    }
    return 0
  }
      ...
      <Page size="A4" style={pdfStyles.page}>
        ...
        <View style={pdfStyles.section}>
          ...
          <View style={pdfStyles.tableHeader}>
            <Text style={[pdfStyles.tableHeaderCell, { width: "30%" }]}>Site</Text>
            <Text style={[pdfStyles.tableHeaderCell, { width: "14%" }]}>Lighthouse</Text>
            <Text style={[pdfStyles.tableHeaderCell, { width: "14%" }]}>Mots</Text>
            <Text style={[pdfStyles.tableHeaderCell, { width: "14%" }]}>Lisibilité</Text>
            <Text style={[pdfStyles.tableHeaderCell, { width: "14%" }]}>Images Alt</Text>
            <Text style={[pdfStyles.tableHeaderCell, { width: "14%" }]}>H1</Text>
          </View>
              ...
              <View key={index} style={pdfStyles.tableRow}>
                ...
                <Text
                  style={[
                    ...
                    result.lighthouseScore === getBestScore("lighthouseScore") ? { fontWeight: "bold" } : {},
                  ]}
                >
                ...
                <Text
                  style={[
                    pdfStyles.metricCell,
                    result.wordCount === getBestScore("wordCount") ? { fontWeight: "bold" } : {},
                  ]}
                >
                  {result.wordCount}
                </Text>
                <Text
                  style={[
                    ...
                    result.readabilityScore === getBestScore("readabilityScore") ? { fontWeight: "bold" } : {},
                  ]}
                >
                ...
                <Text
                  style={[
                    ...
                    altPercentage === getBestScore("altPercentage") ? { fontWeight: "bold" } : {},
                  ]}
                >
                  {altPercentage}%
                </Text>
                <Text
                  style={[
                    ...
                    result.h1Count === getBestScore("h1Count") ? { fontWeight: "bold" } : {},
                  ]}
                >
                  {result.h1Count}
                </Text>
        ...
        <View style={pdfStyles.section}>
          ...
          {findCommonKeywords(results).length > 0 ? (
            ...
            <Text style={{ fontSize: 10, color: "#6b7280" }}>Aucun mot-clé commun trouvé</Text>
          )}
        </View>
        ...
        <View style={pdfStyles.section}>
          ...
          {findCommonIssues(results).length > 0 ? (
            ...
            <Text style={{ fontSize: 10, color: "#6b7280" }}>Aucun problème commun détecté</Text>
          )}
        </View>

        {/* Recommandations par site */}
        <View style={pdfStyles.section}>
          <Text style={pdfStyles.subheader}>Recommandations principales par site</Text>
          {results.map((result, index) => (
            <View key={index} style={{ marginBottom: 10 }}>
              <Text style={{ fontSize: 12, fontWeight: "bold", marginBottom: 5 }}>{result.url}</Text>
              {result.recommendations
                .filter((rec) => rec.severity === "high")
                .slice(0, 3)
                .map((rec, idx) => (
                  <View key={idx} style={{ flexDirection: "row", marginBottom: 3 }}>
                    <Text style={{ fontSize: 8, color: "#ef4444", marginRight: 5 }}>•</Text>
                    <Text style={{ fontSize: 8, flex: 1 }}>{rec.text}</Text>
                  </View>
                ))}
              {result.recommendations.filter((rec) => rec.severity === "high").length === 0 && (
                <Text style={{ fontSize: 8, color: "#22c55e" }}>Aucun problème critique détecté</Text>
              )}
            </View>
          ))}
        </View>
...
export default function ComparisonPDFExport({ results }: { results: SEOAnalysisResult[] }) {
  ...
  const ComparisonPDFDocument = dynamic(() => Promise.resolve(PDFDocument), { ssr: false })
