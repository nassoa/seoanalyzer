...
import type { SEOAnalysisResult, HeadingStructure, ImageAnalysis, LinkAnalysis, StructuredData } from "./types"
...
function analyzeImages($: cheerio.CheerioAPI): ImageAnalysis {
  const images = $("img")
  const imagesList: Array<{ src: string; alt: string | null; hasAlt: boolean }> = []
  let withAlt = 0
  let withoutAlt = 0
...
function extractStructuredData($: cheerio.CheerioAPI): StructuredData[] {
  ...
  if (structuredData.length === 0) {
    const commonTypes = ["Organization", "Person", "WebSite", "Article", "Product", "BreadcrumbList"]
