...
// Apply the autotable plugin to jsPDF to add lastAutoTable property
applyPlugin(jsPDF);
...
export function generateAnalysisPDF(result: SEOAnalysisResult) {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(
      `Rapport généré par Analyseur SEO • Page ${i} sur ${pageCount}`,
      105,
...
export function generateComparisonPDF(results: SEOAnalysisResult[]) {
  ...
  doc.setFontSize(12);
  doc.setTextColor(75, 85, 99); // gris
  doc.text(`Comparaison de ${results.length} sites`, 105, 30, {
    align: "center",
  });
